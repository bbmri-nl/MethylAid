% \VignetteIndexEntry{MethylAid: Visual and interactive quality control of large Illumina 450k data sets}
...
\author{M. van Iterson, E.W. Tobi, R. Slieker, W. den Hollander, R. Luijk,
  \\P.E. Slagboom and B.T. Heijmans
  \\Department of Molecular Epidemiology,
  \\Leiden University Medical Center, Leiden, The Netherlands}

\title{MethylAid: Visual and interactive quality control of large Illumina 450k
  data sets}
...
\Biocpkg{MethylAid} is specially designed for quality control of large sets of
DNA methylation data e.g., epigenomewide association studies (EWAS). Extracting 
intensities from IDAT files can be done in batches and/or in parallel to reduce 
memory load and/or overcome long run-times. It requires two function calls in 
going from IDAT files to launch the interactive web application; 
\Rfunction{summarize} and \Rfunction{visualize}. For more information see 
van Iterson \emph{et alo.} \cite{iterson2014}.
...
summarized data fits into the available RAM. The \Biocexptpkg{minfiData}
provides such a set. The package contains 450k DNA methylation data on 6 samples
across 2 groups. Since, \Biocpkg{MethylAid} uses internally the
\Biocpkg{minfi}\cite{Aryee2014} function \Rfunction{read.450k.exp}, target
information should be provided in a similar way.
...
you see and can do. Enjoy, exploring!

\begin{figure}
   \centering
   \includegraphics[width=\textwidth]{screenshot_MethylAid}
   \caption{Screen shot \Rpackage{MethylAid} interactive web application: The rotated M versus U plot is shown using example data from the package on 500 450k human methylation samples. The colors indicated different scan batches selected using the highlight-metadata column input selector. The dashed vertical line indicates the default filter theshold for the rotated M versus U plot.}
   \label{screenshot_MethylAid}
\end{figure}

\section{Using 450k data downloaded from The Cancer Genome Atlas}
...
a present otherwise these will be removed from the targets file. \fixme{In the
...
resources facilitated by the \Biocpkg{BiocParallel} package.
...
% \section{TODOs}
% \begin{itemize}
% \item add more general reading capabilities
% \item subset data to be visualized using a column and value of the targets file
% \item Functionality for 27k arrays (maybe in old version minfi)
% \item Somehow show NA's in plots
% \item Zoom option filter-plots
% \item Highlight region of samples e.g. using D3 plotting
% \item Search for particular samples
% \item Selected a sample in table and highlight
% \item color removed samples by filter
% \end{itemize}
